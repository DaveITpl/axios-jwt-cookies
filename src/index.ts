...
import { default as StorageCreate, StorageConfigInterface, StorageInterface } from './cookiesStorage'
...
type TokenRefreshRequest = (refreshToken: Token) => Promise<Token | IAuthTokens>
...
export default class AuthProvider {
  ...
  constructor(storageConfig?:StorageConfigInterface) {
    ...
    this.Storage = new StorageCreate(storageConfig||{domain: process.env.MAIN_DOMAIN, storageKey:`auth-tokens-${process.env.NODE_ENV}` });
    this.queue = [];
  }
  ...
  public setAuthTokens = (tokens: IAuthTokens): void =>
    this.Storage.setItem(JSON.stringify(tokens))
   ...
   * Clears both tokens
   */
  public clearAuthTokens = (): void => this.Storage.removeItem()
  ...
  public refreshTokenIfNeeded = async (
    ...
    if (!accessToken || this.isTokenExpired(accessToken)) {
      ...
      accessToken = await this.refreshToken(requestRefresh)
    }
...
// PRIVATE
   ...
   */
  private getAuthTokens = (): IAuthTokens | undefined => {
    const rawTokens = this.Storage.getItem()
    if (!rawTokens) return
   ...
   */
  private getTimestampFromToken = (token: Token): number | undefined => {
   ...
   */
  private getExpiresIn = (token: Token): number => {
   ...
   */
  private refreshToken = async (requestRefresh: TokenRefreshRequest): Promise<Token> => {
    ...
    try {
      ...
      if (typeof newTokens === 'object' && newTokens?.accessToken) {
        ...
        await this.setAccessToken(newTokens)
        return newTokens
      }
...
authTokenInterceptor =
  ({
    ...
    async (requestConfig: AxiosRequestConfig): Promise<AxiosRequestConfig> => {
      ...
      if (!this.getRefreshToken()) return requestConfig
      ...
      let accessToken
      try {
        accessToken = await this.refreshTokenIfNeeded(requestRefresh)
   ...
   */
  private resolveQueue = (token?: Token) => {
   ...
   * @param error Error
   */
  private declineQueue = (error: Error) => {
    this.queue.forEach((p) => {
      p.reject(error)
    })
